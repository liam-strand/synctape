...
} from "../../src/services/spotify-types.ts";
...
describe("SpotifyService", () => {
  ...
  describe("fetchPlaylist", () => {
          ...
          items: [
            {
              ...
              track: {
                ...
                album: { id: "alb1", name: "Album 1", artists: [], images: [], uri: "uri" },
                duration_ms: 200000,
                uri: "uri",
              },
            },
    ...
    it("should handle pagination when fetching a large playlist", async () => {
      const mockPlaylistPage1: Partial<SpotifyPlaylist> = {
        ...
        tracks: {
          items: [{ added_at: "t", track: { name: "Track 1" } } as SpotifyPlaylistTrack],
      ...
      const mockPlaylistPage2: Paginated<SpotifyPlaylistTrack> = {
        items: [{ added_at: "t", track: { name: "Track 2" } } as SpotifyPlaylistTrack],
        next: null,
        total: 2,
        limit: 1,
        offset: 1,
      ...
      expect(fetchMock).toHaveBeenCalledWith("https://api.spotify.com/v1/playlists/playlist123", expect.any(Object));
      expect(fetchMock).toHaveBeenCalledWith("https://api.spotify.com/v1/playlists/playlist123/tracks?offset=1&limit=1", expect.any(Object));
      expect(playlist.name).toBe("Large Playlist");
      expect(playlist.tracks.length).toBe(2);
      expect(playlist.tracks.map(t => t.name)).toEqual(["Track 1", "Track 2"]);
    });
  });
  ...
  describe("createPlaylist", () => {
    it("should fetch user ID and create a new playlist", async () => {
      ...
      const mockPlaylist: Partial<SpotifyPlaylist> = { id: "newPlaylist123", name: "New Playlist", description: "Desc" };
      ...
      const expectedBody = { name: "New Playlist", description: "A new playlist", public: false };
      expect(fetchMock).toHaveBeenCalledWith(
  ...
  describe("updatePlaylistTracks", () => {
    it("should use batching for playlists with more than 100 tracks", async () => {
      ...
      expect(fetchMock).toHaveBeenCalledTimes(2);
      const firstBatchUris = trackIds.slice(0, 100).map(id => `spotify:track:${id}`);
      expect(fetchMock).toHaveBeenCalledWith(
      ...
      const secondBatchUris = trackIds.slice(100).map(id => `spotify:track:${id}`);
      expect(fetchMock).toHaveBeenCalledWith(
        "https://api.spotify.com/v1/playlists/playlist123/tracks",
        expect.objectContaining({
          method: "POST",
